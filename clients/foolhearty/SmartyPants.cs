...
public class SmartyPants : BasePlayerLogic
{
    ...
    public override async Task PlayAsync(CancellationTokenSource cancellationTokenSource)
    {
        ...
        using var refreshCts = CancellationTokenSource.CreateLinkedTokenSource(cancellationTokenSource.Token);
        ...
        try
        {
            while (!cancellationTokenSource.IsCancellationRequested)
            {
                ...
                moveResult = await MoveAsync(direction);

                while (moveResult.newLocation == lastLocation && !cancellationTokenSource.IsCancellationRequested)
                {
                    logger.LogInformation("Didn't move when I went {direction}, trying to go {nextDirection}", direction, tryNextDirection(direction));
                    direction = tryNextDirection(direction);
                    moveResult = await MoveAsync(direction);
                }

                if (!moveResult.ateAPill)
                {
                ...
                while (map.TryGetValue(nextLocation, out var nextCell) && nextCell.isPillAvailable)
                {
                    logger.LogInformation("In a groove! Keep going!");
                    lastRequest = MoveAsync(direction);
    ...
    protected override Location findNearestPlayerToAttack(Location curLocation, List<Cell> board, Location max, Location closest)
    {
        ...
        return new Location(curLocation.row - rowDelta, curLocation.column - columnDelta);
    }
    ...
    private void AdjustThrottling()
    {
        ...
        else if (rateLimitRate < 0.02 && currentDelayMs > 0)
        {
    ...
    private async Task RefreshBoardAsync()
    {
        try
        {
            ...
            var newPlayers = await ExecuteWithRetry(async () =>
                (IReadOnlyList<PlayerInfo>)(await httpClient.GetFromJsonAsync<List<PlayerInfo>>($"{url}/game/{_gameId}/players")
                    ?? new List<PlayerInfo>()));
    ...
    private Task<MoveResult> MoveAsync(string direction)
    {
        return ExecuteWithRetry(async () =>
            await httpClient.GetFromJsonAsync<MoveResult>($"{url}/game/{_gameId}/move/{direction}?token={token}")
    ...
    private async Task<MoveResult> moveFromTo(MoveResult current, Location destination)
    {
        ...
        for (var i = 0; i < Math.Abs(rowDelta); i++)
        {
            result = MoveAsync(direction);
        }

        direction = colDelta < 0 ? "left" : "right";
        for (var i = 0; i < Math.Abs(colDelta); i++)
        {
            result = MoveAsync(direction);
        }
