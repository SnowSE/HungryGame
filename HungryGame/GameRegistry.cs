        ...
        while (true)
        {
            ...
            do
            {
                id = new string(Enumerable.Range(0, length)
                    .Select(_ => Charset[Random.Shared.Next(Charset.Length)])
                    .ToArray());
...
public class GameRegistry
{
    ...
    public GameRegistry(IGameIdStrategy idStrategy, ILogger<GameLogic> gameLogger, IRandomService random, TimeProvider timeProvider)
    {
        ...
        bool isTimed = false, int? timeLimitMinutes = null)
    {
        ...
        var game = new GameLogic(_gameLogger, _random, _timeProvider);
